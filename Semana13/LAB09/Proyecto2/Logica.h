...
class Proyecto2{
	...
	private:
		...
		int icmp; // Indice de cuadrilatero de maypr perimetro
		...
		int getCuadriMayorPerimetro(){
			int indice = 0;
			for(int i=1;i<lista.size();i++){
				if(lista[i].perimetro>lista[indice].perimetro){
					indice = i;
				}
			}
			return indice;
	...
	public:
		...
		void procesarDatos(){
			for(int i=0; i<lista.size(); i++){
				...
				lista[i].perimetro = dosDec(obtenerPerimetro(lista[i]));
				lista[i].area = dosDec(obtenerArea(lista[i]));	
			}
			icmp = getCuadriMayorPerimetro();
		}
		...
		int generarReporte(){
			...
			// Estadistica
			archivo << "\nESTADISTICA\n";
			archivo << "Indice cuadrilatero mayor perimetro: " << lista[icmp].item << endl;
			archivo << "Perimetro : " << lista[icmp].perimetro << endl;
